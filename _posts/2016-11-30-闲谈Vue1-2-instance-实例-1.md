...
Vue.prototype.$delete = function (key) {
  // 依赖于`util/lang`模块中的`del`方法, 用于删除vm上的属性。
  del(this._data, key)
}
```
